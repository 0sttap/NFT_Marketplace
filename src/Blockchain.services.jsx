...
import abi from './abis/PatsoPlaceNFT.json'
...
const updateNFT = async ({ id, cost }) => {
  try {
    cost = window.web3.utils.toWei(cost.toString(), 'ether')
    const contract = await getEtheriumContract()
    const ownerNFT = getGlobalState('connectedAccount')

    await contract.methods.changePrice(Number(id), cost).send({ from: ownerNFT })
  } catch (error) {
    reportError(error)
  }
}
